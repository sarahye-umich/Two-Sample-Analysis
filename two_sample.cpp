...
void print_descriptive_stats(vector<double> data) {
  ...
  if(data.size() < 2){
    cout<<"error: data does not contain at least 2 elements"<<endl;
  }
  else{
...
vector<double> mean_diff_sampling_distribution(
  ...
  for(int i = 0; i<1000; i++){
    a = bootstrap_resample(data_A, i);
    b = bootstrap_resample(data_B, i);

    means = mean(a) - mean(b);

    mean_diff.push_back(means);
  }
  return mean_diff;
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  upper_bound = (0.5 + width/2.0);
  lower_bound = (0.5 - width/2.0);

  double upper;
  double lower;

  upper = percentile(v,upper_bound);
  lower = percentile(v,lower_bound);
